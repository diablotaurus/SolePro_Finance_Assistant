...
class TestTransactionRepository:
    ...
    def test_find_by_counterparty(self, repository, session):
        from solepro.core.domain.entities.transaction import Transaction
        from solepro.core.domain.value_objects.money import Money

        cp1 = CounterpartyModel(name="CP-1")
        cp2 = CounterpartyModel(name="CP-2")
        session.add(cp1)
        session.add(cp2)
        session.commit()
        ...
        tx1 = Transaction(
            ...
            expense=Money(Decimal("0")),
            tax=Money(Decimal("0")),
            counterparty_id=UUID(str(cp1.id)),
            note="cp1",
        )
        tx2 = Transaction(
            date=datetime.now(),
            ...
            expense=Money(Decimal("0")),
            tax=Money(Decimal("0")),
            counterparty_id=UUID(str(cp2.id)),
            note="cp2",
        )
        repository.save(tx1)
        repository.save(tx2)

        results = repository.find_by_counterparty(UUID(str(cp1.id)))
        assert len(results) == 1
        assert results[0].note == "cp1"
    ...
    def test_find_by_type(self, repository):
        ...
        repository.save(Transaction(
            ...
            expense=Money(Decimal("50")),
            tax=Money(Decimal("0")),
            note="expense",
        ))

        income_rows = repository.find_by_type(TransactionType.INCOME)
        expense_rows = repository.find_by_type(TransactionType.EXPENSE)

        assert any(row.note == "income" for row in income_rows)
        assert any(row.note == "expense" for row in expense_rows)

    def test_repository_statistics_and_totals(self, repository):
        from solepro.core.domain.entities.transaction import Transaction
        from solepro.core.domain.value_objects.money import Money
        ...
        assert stats["total_income"] == Decimal("150")
        assert stats["total_expense"] == Decimal("30")
        assert stats["total_tax"] == Decimal("10")
        assert stats["total_profit"] == Decimal("110")

        january = repository.get_monthly_summary(2025, 1)
        assert january["total_count"] == 2

        assert repository.get_total_income().to_decimal() == Decimal("150")
        assert repository.get_total_expense().to_decimal() == Decimal("30")
        assert repository.get_total_tax().to_decimal() == Decimal("10")
        assert repository.get_total_profit().to_decimal() == Decimal("110")
